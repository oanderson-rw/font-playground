...
function updatePageForFontOptions(fontData) {
    ...
    for (const { filename, id, family } of fontData) {
...
/** @type FontFace[] */
const loadableFontFaces = [];
fontsData.forEach((fontData) => {
    const fontFace = new FontFace(
        fontData.family,
        `local(${fontData.family}), url("assets/fonts/${fontData.filename}") format('truetype'), local('Georgia')`,
        {
            style: "normal",
            weight: "normal"
        }
    ...
    loadableFontFaces.push(fontFace);
});

Promise.all(loadableFontFaces.values().map((face) => {
